...
백테스트 상의 모든 수익률 계산은 **수정종가(Adjusted Close)**를 기준으로 하며, 각 자산별 신호 생성에 사용되는 가격 기준은 아래에 명시합니다.
...
2.  **QQQ 동적 운용 (25%):**
    ...
    -   **신호 생성 (`_ma_alignment_weights`):** 매일 **종가(Close)**를 기준으로 QQQ의 5, 10, 20, 60일 이동평균선을 계산하여 투자 신호를 결정하며, 신호 변경 시 리밸런싱이 발생합니다.
        -   **매수 (ON, 비중 1.0):** `5일선 > 10일선 > 20일선 > 60일선`이 모두 충족되는 '완전 정배열' 상태일 때.
        -   **매도 (OFF, 비중 0.0):** `5일선 < 10일선`, `10일선 < 20일선`, `20일선 < 60일선` 중 2개 이상이 충족되는 '역배열' 상태일 때.
        -   **유지:** 위 두 조건에 해당하지 않으면, 이전 상태(ON 또는 OFF)를 그대로 유지합니다.
    -   **재진입 지연 필터:** 잦은 매매를 방지하기 위해, QQQ 매도(OFF) 신호 발생 후 `QQQ_COOLDOWN_DAYS` (기본 30 거래일) 동안은 매수(ON) 신호가 다시 발생해도 무시하고 현금(SGOV) 보유 상태를 유지합니다.
    -   **최종 배분:**
        -   필터까지 통과한 최종 신호가 ON일 경우: QQQ 25% 보유
        -   최종 신호가 OFF일 경우: 해당 25%를 SGOV (단기 국채 ETF)로 전환

3.  **IWD 동적 운용 (25%):**
    -   `LAA_MA2F`와 동일한 로직을 사용합니다.
    -   **신호:** **월말**에 거시 경제 지표(실업률)와 시장 추세(SPY 200일 이평선)를 조합하여 '불경기 & 하락장' 여부를 판단하고, 다음 한 달간 포지션을 유지합니다.
        -   `_is_recession`: 미국 실업률 > 12개월 이동평균
        -   `_is_market_uptrend`: SPY **수정종가(Adjusted Close)** > 200일 이동평균
    -   **배분:**
        -   '불경기'이면서 '하락장'인 구간: IWD를 매도하고 해당 25%를 SGOV로 전환 (리스크 오프)
        -   그 외 모든 구간: IWD 25% 보유 (리스크 온)

4.  **IAU 동적 운용 (25%) - (LAA_MA4의 핵심 변경점):**
    -   **신호:** **월말**에만 아래 두 가지 조건을 동시에 만족하는지 체크하고, 다음 한 달간 포지션을 유지합니다. 수익률 계산은 **수정종가(Adjusted Close)** 기준입니다.
        1.  `IAU`의 최근 1년 수익률 > 0
        2.  `IEF`의 최근 1년 수익률 > 0
    -   **배분:**
